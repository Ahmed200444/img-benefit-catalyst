...
export default function StandaloneTypeformCalculator() {
  ...
  const handleNext = async () => {
    ...
    if (step < STEPS.length - 1) {
      ...
      const payload = {
        ...
        num_employees: n,
        avg_employee_salary: newForm.avg_employee_salary ? parseFloat(newForm.avg_employee_salary) : "",
        avg_marital_status: newForm.avg_marital_status || "",
        gross_revenue_last_year: newForm.gross_revenue_last_year ? parseFloat(newForm.gross_revenue_last_year) : "",
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "#1a73e8" }}>
      <div className="w-full h-1 bg-blue-900/30">
        <div className="h-1 bg-white/60 transition-all duration-500" style={{ width: `${progress}%` }} />
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-10">
        <div className="w-full max-w-xl">
          ...
          {current.hint && <p className="text-white/70 text-sm mb-6">{current.hint}</p>}
